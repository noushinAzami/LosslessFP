...
def extract_data_from_text(text):
    ...
    # Define regex patterns to find the relevant values (compression ratios and throughputs)
    pattern_ratio = r"Geometric mean of median ratios for (.+):\s+Subfolder 1: ([\d.]+)"
    pattern_throughput = r"Geometric mean of median throughputs for (.+):\s+Subfolder 1: ([\d.]+)"

    # Extract compression ratios and throughputs
    ratios = re.findall(pattern_ratio, text)
    throughputs = re.findall(pattern_throughput, text)

    # Adding decompression entries
...
# Extract data
combined_list = extract_data_from_text(text)
