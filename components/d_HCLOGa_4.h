static __device__ inline bool d_HCLOG_4(int& csize, byte in [CS], byte out [CS], byte temp [CS], int& carry, const int chunkID)
{
  ...
  using type = unsigned int;  // must be unsigned
  ...
  const int CB = (32 - __clz((int)sizeof(type))) + 3;  // counter bits
  assert((1 << CB) > TB);
  assert((1 << (CB - 1)) <= TB);
  assert(WS >= SC);
  ...
  for (int i = warp; i < SC; i += warps) {
    ...
    const type max_val = min(max_val1, max_val2);
    if (lane == 0) {
      ...
      int cnt = 0;
      if (max_val != 0) {
        cnt = (sizeof(type) == 8) ? (64 - __clzll(max_val)) : (sizeof(unsigned int) * 8 - __clz((unsigned int)max_val));
      }
  ...
  const int totsize = newsize + extra;
  if (totsize >= CS) return false;
  if (tid == 0) carry = (chunkID == 0) ? totsize : -totsize;
